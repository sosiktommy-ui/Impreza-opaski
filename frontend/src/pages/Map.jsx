...
const braceletColorNames = {
  ...
  GREEN: { label: 'Зелёные', color: '#22c55e' },
  YELLOW: { label: 'Жёлтые', color: '#eab308' },
  PURPLE: { label: 'Фиолетовые', color: '#a855f7' },
  ORANGE: { label: 'Оранжевые', color: '#f97316' },
};

// Helper: aggregate items from transfers
...
// Helper: get sender/receiver label
...
export default function MapPage() {
  ...
  // Total bracelet counts by color
  ...
  // Top routes (city-to-city or location-to-location)
  const topRoutes = useMemo(() => {
    ...
    transfers.forEach((t) => {
      ...
      const key = `${from} → ${to}`;
      if (!routeMap[key]) {
        routeMap[key] = { from, to, count: 0, totalItems: 0, items: {} };
      }
  ...
  // City transfer activity (inbound/outbound counts)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-800">🗺️ Карта трансферов</h2>
        <button
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              <div className="text-2xl font-bold text-brand-600">{transfers.length}</div>
              <div className="text-xs text-gray-500 mt-1">Всего трансферов</div>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              <div className="text-2xl font-bold text-emerald-600">{totalBracelets}</div>
              <div className="text-xs text-gray-500 mt-1">Всего браслетов</div>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              <div className="text-2xl font-bold text-blue-600">{polylines.length}</div>
              <div className="text-xs text-gray-500 mt-1">Маршрутов на карте</div>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              <div className="text-2xl font-bold text-gray-700">{countries.length} / {allCities.length}</div>
              <div className="text-xs text-gray-500 mt-1">Стран / Городов</div>
            </div>
          </div>

          {/* Bracelets by color + Status breakdown */}
                    ...
                    <div className="text-sm min-w-[180px]">
                      ...
                      {act ? (
                        <div className="mt-2 pt-2 border-t border-gray-100 space-y-1">
                          ...
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-500">📦 Браслетов отправлено:</span>
          ...
          {polylines.map((pl) => {
            ...
            return (
              <Polyline
                ...
                color={pl.color}
                weight={Math.min(2 + Math.floor(totalQty / 50), 6)}
                opacity={0.75}
                  ...
                  <div className="text-sm min-w-[200px] space-y-2">
                    ...
                    <div className="text-xs">
                      <span className="text-gray-500">
                        {getLocationLabel(t, 'from')}
                      </span>
                      <span className="mx-1.5 text-gray-300">→</span>
                      <span className="font-semibold text-gray-800">
                        {getLocationLabel(t, 'to')}
                      </span>
                    </div>
                    {items.length > 0 && (
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {items.map((item, idx) => {
                            ...
                            return (
                              <span
                                key={idx}
                                className="inline-flex items-center gap-1 text-[11px] bg-gray-50 rounded px-1.5 py-0.5"
                              >
                                <span className="w-2 h-2 rounded-full" style={{ background: info.color }} />
                    ...
                    {t.notes && (
                      <div className="text-[10px] text-gray-400 italic pt-1 border-t border-gray-100">
                        {t.notes}
                      </div>
                    )}
